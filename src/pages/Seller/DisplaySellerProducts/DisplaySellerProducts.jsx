...
const DisplaySellerProducts = () => {
    ...
    const handleProductPromote = (id) => {
		Swal.fire({
			...
			if (result.isConfirmed) {
				axiosSecure.patch(
					`/promote-product/${user?.uid}?id=${id}`,
					...
					.then((res) => {
						...
						Swal.fire({
                            ...
                            text: data.message || 'The product has been promoted successfully.',
                            
                        });
						refetch();
					})
					.catch((err) => {
                        const errorMessage = err.response?.data?.message || err.message || 'Oops! Something went wrong, please try again';
						Swal.fire({
                            title: 'Error',
                            text: errorMessage,
